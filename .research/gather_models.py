...
def get_final_results(log_json_path, iter_num):
    ...
    with open(log_json_path, 'r') as f:
        for line in f.readlines():
            ...
            # steps on single gpu.
            flag1 = ('aji' in log_line) or (log_line['mode'] == 'val')
...
def get_best_results_iters(log_json_path):
    best_results = 0
    result_dict = {}
    best_iter = 0
    last_iter = 0
    with open(log_json_path, 'r') as f:
        for line in f.readlines():
            ...
            # extract evaluation results when evaluation mode iteration
            flag1 = ('aji' in log_line) or (log_line['mode'] == 'val')
            if flag1:
                # This bug has been fixed
                if isinstance(log_line['aji'], list):
                    log_line['aji'] = log_line['aji'][0]
                    log_line['dice'] = log_line['dice'][0]

                if log_line['aji'] > best_results:
                    best_results = log_line['aji']
                    best_iter = last_iter + 50
...
def main():
    ...
    for used_config in used_configs:
        ...
        best_iter = final_iter
        for idx, _log_json_path in enumerate(log_json_paths):
            best_model_performance, best_iter = get_best_results_iters(
                _log_json_path)
            final_model_performance = get_final_results(
                _log_json_path, final_iter)
            if final_model_performance is not None:
                log_json_path = _log_json_path
                break

        if final_model_performance is None:
        ...
        model_infos.append(
            dict(
                ...
                final_results=final_model_performance,
                final_iters=final_iter,
                best_results=best_model_performance,
    ...
    for model in model_infos:
        ...
        trained_model_path = osp.join(
            work_dir, model_name, config_name,
            'iter_{}.pth'.format(model['final_iters']))
