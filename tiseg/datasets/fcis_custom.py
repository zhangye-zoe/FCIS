...
def draw_all(save_folder,
    ...
    # print("tc_sem_pred", tc_sem_pred.shape, np.unique(tc_sem_pred))
    # print("tc_sem_gt", tc_sem_gt.shape, np.unique(tc_sem_gt))

    plt.subplot(247)
    plt.imshow(colorize_seg_map(tc_sem_pred, sem_palette)) #tc_palette))
    plt.axis('off')
...
class FCISCustomDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.data_infos = self.load_annotations(self.img_dir, self.ann_dir, self.adj_dir, self.dis_dir, self.img_suffix, self.sem_suffix,
                                                self.inst_suffix, self.adj_suffix, self.dis_suffix, self.split)
    ...
    def load_annotations(self, img_dir, ann_dir, adj_dir, dis_dir, img_suffix, sem_suffix, inst_suffix, adj_suffix, dis_suffix, split=None):
    ...
    def pre_eval(self, preds, indices, show=False, show_folder=None):
        ...
        for pred, index in zip(preds, indices):
            ...
            # Metric calculation.
            # Dice in the paper is binary foreground Dice.  Do not evaluate the
            # four-color map directly as a two-class semantic map.
            ...
            empty_correct = int(gt_empty and pred_empty)

            # semantic metric calculation (foreground class only)
            ...
            # instance metric calculation.  Empty-empty samples are converted to
            # NaN in image-wise AJI/DQ/SQ/PQ/InstDice and ignored by nanmean.
    ...
    def evaluate(self, results, logger=None, **kwargs):
        ...
        # Empty-patch statistics.  These are reported separately and are not
        # folded into DQ/SQ/PQ, because empty-empty samples have no detection
        # target and should not be counted as zero-quality detections.
        ...
        ret_metrics['EmptyAcc'] = np.mean(empty_correct_list[empty_gt_list]) if np.any(empty_gt_list) else np.nan
        ...
        for key in ['Aji', 'DQ', 'SQ', 'PQ', 'InstDice']:
            if key in img_ret_metrics:
                arr = np.asarray(img_ret_metrics[key], dtype=np.float32)
                arr[empty_empty_list] = np.nan
                img_ret_metrics[key] = arr
        ...
        for key in ['bAji', 'bDQ', 'bSQ', 'bPQ', 'EmptyAcc', 'NumEmptyGT', 'NumEmptyPred']:
        ...
        # mean table. Metric values are displayed as percentages.
        ...
        overall_metrics = OrderedDict({
            sem_key: (np.round(np.nanmean(value), 2) if sem_key.startswith('Num')
