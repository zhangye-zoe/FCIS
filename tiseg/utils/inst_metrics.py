...
def pre_eval_to_bin_aji(pre_eval_results, nan_to_num=None):
    ...
    union_sum = sum(inst_union)
    ret_metrics = {'Aji': np.nan if union_sum == 0 else sum(inst_inter) / union_sum}
...
def pre_eval_to_imw_aji(pre_eval_results, nan_to_num=None):
    ...
    for i, u in zip(inst_inter, inst_union):
        ret_metrics['Aji'].append(np.nan if u == 0 else i / u)
...
def pre_eval_to_bin_pq(pre_eval_results, nan_to_num=None, analysis_mode=False):
    ...
    # Empty-empty samples contain no detection target and no error.  Return NaN
    # for image-wise aggregation; dataset-level aggregation remains unaffected
    # when non-empty samples exist.
    if denom == 0:
        dq = np.nan
        sq = np.nan
        pq = np.nan
        ...
        dq = tp / denom
        sq = np.nan if tp == 0 else iou / (tp + 1.0e-6)
        pq = 0.0 if np.isnan(sq) else dq * sq
