...
class FCISNet(BaseSegmentor):
    ...
    def forward(self, data, label=None, metas=None, **kwargs):
        ...
        if self.training:
            ...
            # Calculate the orthological loss between adjacent cells.
            # Use a config-controlled sample ratio for reproducibility.
            ortho_sample_ratio = self.train_cfg.get('ortho_sample_ratio', 0.5)
            ortho = OrthoLoss(ins_gt, adj_gt, sem_gt, inter_pred, sample_ratio=ortho_sample_ratio)
            ...
            # `fc_pred` keeps the four-color prediction for visualization/debugging.
            # `sem_pred` is the binary foreground mask used by the paper's Dice metric.
            fc_pred, inst_pred = self.postprocess(cls_pred)
            ...
            # unravel batch dim
            ret_list = []
            ret_list.append({'sem_pred': sem_pred, 'fc_pred': fc_pred, 'inst_pred': inst_pred})
            return ret_list
    ...
    def postprocess(self, pred):
        ...
        for sem_id in sem_id_list:
            ...
            # Fill small holes inside predicted nuclei and remove tiny fragments.
            hole_size = self.test_cfg.get('hole_size', 16)
            min_size = self.test_cfg.get('min_size', 10)
            sem_id_mask = binary_fill_holes(sem_id_mask)
            sem_id_mask = remove_small_holes(sem_id_mask.astype(bool), area_threshold=hole_size)
            sem_id_mask = remove_small_objects(sem_id_mask, min_size=min_size)
            inst_sem_mask = measure.label(sem_id_mask)

            radius = self.test_cfg.get('radius', 1)
            if radius > 0:
                # Support both old and new skimage APIs.
                try:
                    inst_sem_mask = morphology.dilation(inst_sem_mask, footprint=morphology.disk(radius))
                except TypeError:
                    inst_sem_mask = morphology.dilation(inst_sem_mask, selem=morphology.disk(radius))

            inst_sem_mask[inst_sem_mask > 0] += cur
            inst_pred[inst_sem_mask > 0] = 0
            inst_pred += inst_sem_mask
            cur += max(len(np.unique(inst_sem_mask)) - 1, 0)
    ...
    def _cls_loss(self, sem_logit, sem_gt, weight_map=None):
        """Four-color foreground classification loss.

        The paper defines this loss only on foreground pixels.  The original
        implementation multiplied the pixel-wise CE by the foreground mask and
        then averaged over the whole image, which diluted the supervision when
        the foreground area was small.  Here the loss is normalized by the
        number of foreground pixels.
        """
        sem_loss = {}
        if weight_map is None:
            fg_mask = (sem_gt > 0).float()
        else:
            fg_mask = (weight_map > 0).float()

        fg_count = fg_mask.sum().clamp_min(1.0)

        sem_ce_loss_calculator = nn.CrossEntropyLoss(reduction='none')
        sem_ce_loss = sem_ce_loss_calculator(sem_logit, sem_gt)
        sem_ce_loss = (sem_ce_loss * fg_mask).sum() / fg_count

        sem_dice_loss = self._foreground_dice_loss(sem_logit, sem_gt, fg_mask)

        alpha = self.train_cfg.get('cls_ce_weight', 10.0)
        beta = self.train_cfg.get('cls_dice_weight', 1.0)
        sem_loss['cls_ce_loss'] = alpha * sem_ce_loss
        sem_loss['cls_dice_loss'] = beta * sem_dice_loss

        return sem_loss

    def _foreground_dice_loss(self, sem_logit, sem_gt, fg_mask, smooth=1e-4):
        """Masked Dice loss for four-color classes on foreground pixels only."""
        pred_prob = F.softmax(sem_logit, dim=1)
        target_one_hot = F.one_hot(sem_gt, num_classes=sem_logit.shape[1]).permute(0, 3, 1, 2).float()
        fg_mask = fg_mask.unsqueeze(1)

        loss = sem_logit.new_tensor(0.0)
        valid_classes = 0
        for c in range(1, sem_logit.shape[1]):
            pred_c = pred_prob[:, c:c + 1] * fg_mask
            target_c = target_one_hot[:, c:c + 1] * fg_mask
            denom = pred_c.sum() + target_c.sum()
            if denom.detach() <= 0:
                continue
            dice = (2.0 * (pred_c * target_c).sum() + smooth) / (denom + smooth)
            loss = loss + (1.0 - dice)
            valid_classes += 1
        if valid_classes == 0:
            return sem_logit.new_tensor(0.0)
        return loss / valid_classes

    def _intra_instance_consistency_loss(self, sem_logit, inst_gt, max_instances=32, max_pixels=256):
        """Reduce color fragmentation inside the same nucleus.

        For each sampled GT instance, the four-color probability vectors should
        be internally consistent.  This is intentionally lightweight so it can be
        used during training without large memory overhead.
        """
        prob = F.softmax(sem_logit, dim=1)
        total_loss = sem_logit.new_tensor(0.0)
        count = 0
        batch_size = inst_gt.shape[0]

        for b in range(batch_size):
            ids = torch.unique(inst_gt[b])
            ids = ids[ids > 0]
            if ids.numel() == 0:
                continue
            ...
            for inst_id in ids:
                coords = torch.nonzero(inst_gt[b] == inst_id, as_tuple=False)
                if coords.size(0) <= 1:
                    continue
                if coords.size(0) > max_pixels:
                    perm = torch.randperm(coords.size(0), device=coords.device)[:max_pixels]
                    coords = coords[perm]
                feat = prob[b, 1:, coords[:, 0], coords[:, 1]].transpose(0, 1)
                total_loss = total_loss + feat.var(dim=0, unbiased=False).mean()
                count += 1
