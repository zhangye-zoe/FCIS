...
class FCISNet4(BaseSegmentor):
    ...
    def calculate(self, img, encoding):
        ...
        mask_feature, sem_pred, dis_pred = self.head(bottom_feat, skip_feats, encoding)
        # print("dis pred shape", dis_pred.shape)

        # import matplotlib.pyplot as plt
        # plt.figure()
        # plt.imshow(dis_pred[0,0,...].cpu().numpy())
        # plt.show()
        # plt.savefig("z_h_distance.png")

        # plt.figure()
        # plt.imshow(dis_pred[0,1,...].cpu().numpy())
        # plt.show()
        # plt.savefig("z_v_distance.png")

        return mask_feature, sem_pred, dis_pred

    def forward(self, data, label=None, metas=None, **kwargs):
        """detectron2 style forward functions. Segmentor can be see as meta_arch of detectron2.
        """
        print("Corresponding Functions", dir(self))
        print("=" * 100)
        if self.training:
            sem_feature, sem_pred, dis_pred = self.calculate(data['img'], None) # self.position_encoding)

            fore_pred = sem_pred[:, 1:, ...].sum(dim=1, keepdim=True)
            back_pred = sem_pred[:, :1, ...]
            bi_sem_pred = torch.cat([back_pred, fore_pred], dim=1)
            ...
            # dis_gt = label['dis_gt']
            adj_gt = label['adj_gt']
            # print("inst gt", torch.unique(ins_gt))
            # print("adj gt", adj_gt )
            # print("=" * 100)
            ...
            sole_sem_gt = sem_gt_wb.clone().long()
            sole_sem_gt[sem_gt_wb > 2] = 2
            # import matplotlib.pyplot as plt
            # plt.figure()
            # plt.imshow(bi_sem_gt[0,...].cpu().numpy())
            # plt.show()
            # plt.savefig("z_sem_gt.png")
            # print("unique", torch.unique(bi_sem_gt))

            # plt.figure()
            # plt.imshow(bi_sem_pred[0,0,...].detach().cpu().numpy())
            # plt.show()
            # plt.savefig("z_back_logit.png")

            # plt.figure()
            # plt.imshow(bi_sem_pred[0,1,...].detach().cpu().numpy())
            # plt.show()
            # plt.savefig("z_fore_logit.png")


            # import matplotlib.pyplot as plt
            # print("sem gt", sem_pred.shape)
            # plt.imshow(sem_pred[0,0,...].detach().cpu().numpy())
            # plt.savefig("z_sem_pred.png")
            # plt.figure()
            # plt.imshow(sem_gt[0,0,...].cpu().numpy())
            # plt.savefig("z_sem_gt.png")
            # plt.figure()
            # plt.imshow(dis_gt[0,...,0].detach().cpu().numpy())
            # plt.savefig("z_dis_gt.png")
       

            loss = dict()
            
            # Calculate the foreground and background semantic loss
            # print("sem pred", bi_sem_pred.shape)
            # print("sem gt", bi_sem_gt.shape)
            # sem_loss = self._sem_loss(bi_sem_pred, bi_sem_gt)
            # loss.update(sem_loss)
            # print("sem pred", sem_pred.shape)
            # print("sem gt", sem_gt.shape)

            # print("=" * 100)

            sem_loss = self._sem_loss(sem_pred, sole_sem_gt)
            loss.update(sem_loss)

            # Calculate the ortholodical loss inner and inter cells
            ortho = OrthoLoss(ins_gt, adj_gt, sem_gt, sem_pred)
            ortho_loss = ortho.calculate_ortho_loss()
            loss.update(ortho_loss)

            # Calculate the distance loss
            # dis_loss = self._dis_loss(dis_pred, dis_gt.permute(0,3,1,2), bi_sem_gt)
